    ...
    const Option<Principal>& principal) const
{
  ...
  if (
    (containerId.isSome() && !(executorId.isNone() && frameworkId.isNone())) ||
    (containerId.isNone() && executorId.isSome() && frameworkId.isNone())
    ) {
    return Failure(
      "Query parameters did not match expectation: input either container id "
      "only, framework id only, or executor and framework id only");
  }
    ...
    .then(defer(slave->self(), [=](const hashset<ContainerID> containerIds) {
      ...
      foreachvalue (const Framework* framework, slave->frameworks) {
        ...
        foreachvalue (const Executor* executor, framework->executors) {
          ...
          if (
            (selectContainerId.isSome() &&
             !selectContainerId->accept(containerId)) ||
            (selectExecutorId.isSome() &&
             !selectExecutorId->accept(executorId)) ||
