...
Cgroups2NvidiaGpuIsolatorProcess::Cgroups2NvidiaGpuIsolatorProcess(
  const Flags& _flags,
  const NvidiaGpuAllocator& _allocator,
  const NvidiaVolume& _volume,
  const map<Path, cgroups::devices::Entry>& _controlDeviceEntries,
...
Try<Isolator*> Cgroups2NvidiaGpuIsolatorProcess::create(
    const Flags& flags,
    const NvidiaComponents& components,
    ...
    )
{
  ...
  process::Owned<MesosIsolatorProcess> process(
      new Cgroups2NvidiaGpuIsolatorProcess(
          flags,
          components.allocator,
          components.volume,
          deviceEntries,
...
bool Cgroups2NvidiaGpuIsolatorProcess::supportsNesting()
{
  return false;
}


bool Cgroups2NvidiaGpuIsolatorProcess::supportsStandalone()
{
  return true;
}


Future<Nothing> Cgroups2NvidiaGpuIsolatorProcess::recover(
    const vector<ContainerState>& states,
    const hashset<ContainerID>& orphans)
{
  vector<Future<Nothing>> futures;
  ...
  foreach (const ContainerState& state, states) {
    ...
    // Determine which GPUs are allocated to this container.
    vector<cgroups::devices::Entry> entries =
      deviceManager->list(cgroup);
    
    // entries_future.await();
    
    // if (entries_future.isFailed()) {
    //   return Failure("Failed to obtain devices list for cgroup"
    //                  " '" + cgroup + "': " + entries_future.failure());
    // }

    // vector<cgroups::devices::Entry> entries = entries_future.get();
...
Future<Option<ContainerLaunchInfo>> Cgroups2NvidiaGpuIsolatorProcess::prepare(
    const ContainerID& containerId,
    const mesos::slave::ContainerConfig& containerConfig)
{
  if (containerId.has_parent()) {
  ...
  vector<cgroups::devices::Entry> control_device_entries_vector;

  foreachkey (const Path& devicePath, controlDeviceEntries) {
    control_device_entries_vector.push_back(
      controlDeviceEntries.at(devicePath));
  }

  Future<Nothing> configure_status = deviceManager->configure(
    infos[containerId]->cgroup, control_device_entries_vector, {});

  configure_status.await();

  if (configure_status.isFailed()) {
    return Failure(
      "Failed to grant cgroups access to control devices when preparing "
      "container " +
      containerId.value() + " :" + configure_status.failure());
  }

  return update(containerId, containerConfig.resources())
    .then(defer(PID<Cgroups2NvidiaGpuIsolatorProcess>(this),
                &Cgroups2NvidiaGpuIsolatorProcess::_prepare,
                containerId,
                containerConfig));
}
...
// host file system, then we need to prepare a script to inject our
// `NvidiaVolume` into the container (if required).
Future<Option<ContainerLaunchInfo>> Cgroups2NvidiaGpuIsolatorProcess::_prepare(
    const ContainerID& containerId,
    const mesos::slave::ContainerConfig& containerConfig)
{
  if (!containerConfig.has_rootfs()) {
...
Future<Nothing> Cgroups2NvidiaGpuIsolatorProcess::update(
    const ContainerID& containerId,
    const Resources& resourceRequests,
    const google::protobuf::Map<string, Value::Scalar>& resourceLimits)
{
  ...
  if (requested > info->allocated.size()) {
    ...
    return allocator.allocate(additional)
      .then(defer(PID<Cgroups2NvidiaGpuIsolatorProcess>(this),
                  &Cgroups2NvidiaGpuIsolatorProcess::_update,
                  containerId,
                  lambda::_1));
  } else if (requested < info->allocated.size()) {
    size_t fewer = info->allocated.size() - requested;
    ...
    vector<cgroups::devices::Entry> gpus_for_deallocation;
    ...
    for (size_t i = 0; i < fewer; i++) {
      ...
      gpus_for_deallocation.push_back(entry);
      deallocated.insert(*gpu);
      info->allocated.erase(gpu);
    }

    Future<Nothing> configure_status =
        deviceManager->configure(info->cgroup, {}, {gpus_for_deallocation});

    configure_status.await();

    if (configure_status.isFailed()) {
      return Failure(
        "Failed to deny cgroups access to GPU device: " +
        configure_status.failure());
    }
...
Future<Nothing> Cgroups2NvidiaGpuIsolatorProcess::_update(
    const ContainerID& containerId,
    const set<Gpu>& allocation)
{
  if (!infos.contains(containerId)) {
  ...
  Info* info = CHECK_NOTNULL(infos.at(containerId));
  vector<cgroups::devices::Entry> gpus_for_allocation;
  ...
  foreach (const Gpu& gpu, allocation) {
    ...
    gpus_for_allocation.push_back(entry);
  }

  Future<Nothing> configure_status =
      deviceManager->configure(info->cgroup, {gpus_for_allocation}, {});
...
Future<ResourceStatistics> Cgroups2NvidiaGpuIsolatorProcess::usage(
    const ContainerID& containerId)
{
  if (containerId.has_parent()) {
    return Failure("Not supported for nested containers");
...
Future<Nothing> Cgroups2NvidiaGpuIsolatorProcess::cleanup(
    const ContainerID& containerId)
{
  // If we are a nested container, we don't have an `Info()` struct to
  // cleanup, so we just return immediately.
