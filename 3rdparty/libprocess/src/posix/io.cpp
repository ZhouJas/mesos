...
void Watcher::run()
{
  ...
  data->read_loop = loop(
      ...
      [weak_data, buffer](size_t read) -> Future<ControlFlow<Nothing>> {
        ...
        shared_ptr<Data> converted_data = weak_data.lock();
        if (!converted_data) {
          return Break();
        }
        ...
        for (offset = 0; offset <= read - sizeof(inotify_event);) {
          ...
          Option<string> path = converted_data->wd_to_path.get(event->wd);
          ...
          if (event->mask & IN_MOVE_SELF) {
            e.type = Event::Rename;
            converted_data->wd_to_path.erase(event->wd);
            converted_data->path_to_wd.erase(*path);
          }
...
Try<Nothing> Watcher::add(const string& path)
{
  ...
  int mask = IN_CLOSE_WRITE | IN_DELETE_SELF | IN_MOVE_SELF;
